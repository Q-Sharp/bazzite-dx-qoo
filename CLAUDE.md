...
- **`build_files/build.sh`**: the place to install packages or make image modifications. It first copies `system_files/` onto `/`, then installs packages, then builds the kernel module. The module build queries the image's kernel version via `rpm -q kernel`, installs `kernel-devel-matched` temporarily, compiles, signs with `/ctx/MOK.priv` + `/ctx/MOK.der`, installs to `/usr/lib/modules/<kver>/extra/`, runs `depmod`, and removes the build deps again.
- **`system_files/`**: overlay copied verbatim onto the image root (`usr/lib/modules-load.d/nct6687.conf` loads the module at boot; `usr/lib/modprobe.d/nct6687.conf` sets `force=true`).
...
- **`build.yml`**: runs on push to main, PRs, daily at 10:05 UTC, and manual dispatch. Pipeline: `just check` → write `MOK.priv` from secret → `sudo just build` → `just rechunk` (Chunkah) → tag → push to GHCR → cosign sign. Push/sign steps only run on the default branch, never on PRs.
